...
class BaseTest(TestCase):
    ...
    def check_record(self, data_sent, data_received, parsed_model, orig_data=None):
        """
        ...
        :param record: A dictionary with the values returned by the endpoint
        ...
        for key in parsed_model['keys']:
            if orig_data and key in parsed_model['excluded']:
                data_sent[key] = orig_data[key]
            if key in data_received and key in data_sent:
                if key is 'password_hash':
                    self.assertTrue(check_password_hash(data_received['password_hash'], data_sent['password']))
                elif key in parsed_model['int']:
                    self.assertEqual(data_sent[key], int(data_received[key]))
                elif key in parsed_model['float']:
                    self.assertEqual(data_sent[key], float(data_received[key]))
                elif key in parsed_model['bool'] and data_sent[key]:
                    self.assertTrue(data_received[key])
                elif key in parsed_model['bool'] and not data_sent[key]:
                    self.assertFalse(data_received[key])
                else:
                    self.assertEqual(data_sent[key], data_received[key])
